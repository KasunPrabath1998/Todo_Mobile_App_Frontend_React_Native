...
const ProfileScreen = () => {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      try {
        ...
        if (token && userId) {
          const response = await axios.get(`http://54.237.53.166:3001/api/users/${userId}`, {
            headers: {
  ...
  const handleUpdate = async () => {
    try {
      ...
      if (token && userId) {
        await axios.put(
          `http://54.237.53.166:3001/api/users/profile/${userId}`,
          {
            ...newUserDetails,
        ...
        if (selectedAsset.uri) {
          ...
          if (token && userId) {
            const uploadResponse = await axios.post(
              `http://54.237.53.166:3001/api/upload-profile-image/${userId}`,
              formData,
              {
                headers: {
                  Authorization: `Bearer ${token}`,
