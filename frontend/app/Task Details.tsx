...
const TodoDetailScreen: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTodoDetails = async () => {
      try {
        ...
        const response = await axios.get(`http://54.237.53.166:3001/api/todos/${userId}/${todoId}`);  
  ...
  const handleUpdate = async () => {
    try {
      ...
      const response = await axios.put(`http://54.237.53.166:3001/api/todos/${userId}/${todoId}`, { 
        title: updatedTitle,
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              const response = await axios.delete(`http://54.237.53.166:3001/api/todos/delete/${todoId}`);
              if (response.status === 200) {
